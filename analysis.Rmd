...
```{r}
  ...
  teamsDf <- teamsDf %>% group_by(tmID) %>% summarise(W = sum(W), L = sum(L), G=sum(G))
...
```{r}
mostWins %>% select(tmID) %>% semi_join(bestWinRatio %>% select(tmID)) %>% semi_join(bestLossRatio %>% select(tmID))
